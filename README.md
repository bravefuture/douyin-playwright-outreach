...
- Optionally checks the visible Douyin DM inbox for unread indicators.
...
Check unread DMs once, or keep watching until interrupted:

```bash
npm run monitor -- --enabled true --out /tmp/douyin-inbox-check.json
npm run monitor -- --enabled true --realtime --interval-ms 30000 --out /tmp/douyin-inbox-watch.json
...
- `--login-timeout-ms <ms>`: maximum wait time for manual login or verification. The script resumes earlier when verification UI disappears for consecutive checks. Default: `300000`.
- `--manual-auth-poll-ms <ms>`: read-only polling interval while the user is completing login or verification. Default: `2000`.
- `--manual-auth-stable-checks <number>`: number of consecutive clear checks required before automation resumes. Default: `3`.
- `--post-auth-wait-ms <ms>`: short wait after login/verification disappears before retrying. Default: `1500`.
- `--post-auth-retry-wait-ms <ms>`: short wait after retrying the original action. Default: `1800`.
...
- `--send-delay-min-ms <ms>`: minimum random delay before the next approved creator. Default: `3000`.
- `--send-delay-max-ms <ms>`: maximum random delay before the next approved creator. Default: `10000`.
- `--no-send-delay`: disable random delay between approved creators. Not recommended for real outreach.
- `--out <path>`: write JSON output to a file.

### `monitor`

Checks the visible Douyin DM inbox for unread indicators. The monitor waits for the inbox title or conversation rows to load and stabilize before each check. This is a rendered-page best-effort check; the user should confirm important replies in the browser.

In realtime mode, transient inbox load failures are logged and retried instead of stopping the monitor. The monitor hovers the top-bar DM entry and keeps the mouse inside the inbox panel while checking, because Douyin may close the inbox popover when the pointer leaves it. Use `--enabled false` or stop the process to turn monitoring off.

```bash
npm run monitor -- --enabled true --out /tmp/douyin-inbox-check.json
npm run monitor -- --enabled true --realtime --interval-min-ms 30000 --interval-max-ms 60000
npm run monitor -- --enabled false
```

Useful options:

- `--enabled true|false`: turns unread monitoring on or off for this run. Default: `true`.
- `--watch` / `--realtime`: keep checking instead of running once.
- `--interval-min-ms <ms>`: minimum random wait between realtime checks. Default: `30000`.
- `--interval-max-ms <ms>`: maximum random wait between realtime checks. Default: `60000`.
- `--interval-ms <ms>`: fixed wait between checks. Overrides random interval when set.
- `--duration-ms <ms>`: stop watch mode after this duration. Default: `0` means run until interrupted.
- `--im-entry-timeout-ms <ms>`: maximum wait for the top-bar DM entry to appear before opening the inbox. Default: `60000`.
- `--inbox-load-timeout-ms <ms>`: maximum wait for the inbox list to load and stabilize. Default: `20000`.
- `--inbox-stable-checks <number>`: consecutive stable inbox snapshots required before reading. Default: `2`.
- `--emit-all`: print every realtime check. By default realtime mode prints the first check, changes, and unread detections.
- `--out <path>`: write the final monitor log to a file.
...
Row-level failures such as `failed_web_send`, `failed_dm_button_unavailable`, `failed_editor_unavailable`, `failed_draft_mismatch`, and `send_unconfirmed` are recorded and the sender continues to the next approved row. The run stops only for global states such as manual-auth timeout, account abnormality, repeated-operation/rate-limit warnings, or platform-wide send restrictions.
...
- Continue past creator-specific privacy, unavailable DM, draft mismatch, send failure, or unconfirmed-send rows when later rows are already approved.
- Stop on rate limits, repeated operation warnings, account abnormality, platform-wide send restrictions, or unclear account-level state.
